...
CREATE TABLE outro_deliveries (
    delivery_id INTEGER PRIMARY KEY,
    user_id INTEGER NOT NULL UNIQUE,
    kind TEXT NOT NULL CHECK (kind IN ('success', 'timeout')),
    trigger_at_ms INTEGER NOT NULL CHECK (trigger_at_ms >= 0),
...
CREATE INDEX outro_deliveries_status_idx
    ON outro_deliveries(status, next_attempt_at_ms, delivery_id);

CREATE TABLE outro_delivery_parts (
    ...
    FOREIGN KEY (delivery_id) REFERENCES outro_deliveries(delivery_id) ON DELETE CASCADE
) STRICT, WITHOUT ROWID;

CREATE INDEX outro_delivery_parts_ready_idx
    ON outro_delivery_parts(status, next_attempt_at_ms, delivery_id, part_number);
